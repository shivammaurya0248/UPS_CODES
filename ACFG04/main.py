...
def main():
    ...
    while True:
        ...
        if AMQP_DATA in ["Send Trolley Weight", "SEND SCRAP PIECE", "SEND END PIECE", "SEND TRIMMING"]:
            ...
            try:
                weight = read_weight()
                # Skip sending weights greater than 600
                if weight > THRESHOLD_WEIGHT:
                    log.warning(f"[-] Weight {weight} exceeds the threshold ({THRESHOLD_WEIGHT}) and will not be sent.")
                    continue
        ...
        if not FL_SPIKE_OCCURRED:
            if FL_PRODUCTION_START or FL_PRODUCTION_PREV_STATUS:
                if (time.time() - GL_LAST_PROD_SEND_TIME) > PRODUCTION_SAMPLE_RATE_SEC or FL_PRODUCTION_START != FL_PRODUCTION_PREV_STATUS:
                    # Skip sending weights greater than 600
                    if GL_WEIGHT_TO_SEND > THRESHOLD_WEIGHT:
                        log.warning(f"[-] GL_WEIGHT_TO_SEND {GL_WEIGHT_TO_SEND} exceeds the threshold ({THRESHOLD_WEIGHT}) and will not be sent.")
                        continue
